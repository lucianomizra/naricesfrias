...
class ArticleModel extends AppModel {
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($text)
      $sql .= " AND ( t.title like '%{$text}%'  OR  t.article like '%{$text}%'  OR t.id = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  public function SavePost()
  {
    ...
    $data = array(
      ...
      'article' => $this->input->post('article'),
        ...
        if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    $ret['article'] = $this->input->post() ? $this->input->post('article') : '';
    return $ret;
  }
